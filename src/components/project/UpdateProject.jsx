...
const UpdateProject = ({project,title,open,setOpen}) => {
    const classes = useStyles()
    const [clave, setClave] = useState(project.code)
    const [nombre, setNombre] = useState(project.name)
    const [fechaInicio, setFechaInicio] = useState(String(project.startDate).substring(0,10))
    const [fechaFin, setFechaFin] = useState(String(project.finishDate).substring(0,10))
    const statusDate = new Date(project.statusDate)
    const handleUpdateProject = async () => {
        ...
        console.log(data)
        try {          
            const response=await api.projects.update(data);
    ...
    return (
        ...
        <DialogTitle>{title}</DialogTitle>
          ...
          <DialogActions>
              ...
              <Col>
                <Button onClick={()=>{handleUpdateProject();setOpen(false)}} type="button" 
